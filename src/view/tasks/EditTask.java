...
public class EditTask extends JDialog {
	...
	//Create the Dialog
	public EditTask(MainMenu caller, String name) {
		setResizable(false);
		...
		dateLbl.setForeground(new Color(252, 252, 252));
		dateLbl.setFont(new Font("Lucida Grande", Font.PLAIN, 14));
		...
		try {
			...
			nameField.setText(name.split(" - ")[1]);
			notesArea.setText(cont.getInfoTask(name));
			if (!name.split(" - ")[0].equals("Ind")) {
				dateChooser.setDate(new SimpleDateFormat("yyyy/MM/dd").parse(name.split(" - ")[0]));
			}
				...
				try {
					if (nameField.getText().trim().equals("") || nameField.getText() == null) {
						...
						if (!correct) {
							...
							CtrlTasks cont = new CtrlTasks();
							if (!(name.trim().replaceAll(" ", "").toLowerCase().equals(nameField.getText().trim().replaceAll(" ", "").toLowerCase())) && !cont.checkTask(nameField.getText())) {
								...
								if (cont.removeTask(name)) {
									if (cont.addTask(task)) {
										...
										caller.rightMessage("Tarea editada correctamente!");
										dispose();
									} else {
										errorMessage("INTERNAL ERROR!");
									}
				...
				.addGroup(gl_contentPanel.createSequentialGroup()
					...
					.addGroup(gl_contentPanel.createParallelGroup(Alignment.TRAILING)
						...
						.addGroup(Alignment.LEADING, gl_contentPanel.createSequentialGroup()
							.addComponent(dateLbl, GroupLayout.DEFAULT_SIZE, 283, Short.MAX_VALUE)
							.addGap(6)
							.addComponent(dateChooser, GroupLayout.DEFAULT_SIZE, 288, Short.MAX_VALUE)
							.addGap(297)))
					.addContainerGap())
		);
			...
			gl_contentPanel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_contentPanel.createSequentialGroup()
					...
					.addGroup(gl_contentPanel.createParallelGroup(Alignment.LEADING)
						.addComponent(dateLbl, GroupLayout.PREFERRED_SIZE, 27, GroupLayout.PREFERRED_SIZE)
